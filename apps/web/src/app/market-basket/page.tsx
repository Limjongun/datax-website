...
import { ShoppingCart, PlusCircle, Play } from 'lucide-react';
...
export default function MarketBasketPage() {
  ...
  const runAnalysis = async () => {
    setIsLoading(true);
    try {
      const res = await fetch('http://localhost:8000/api/stats/market-basket', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          file_path: filePath,
          transaction_id_col: transactionIdCol || columns[0],
          item_id_col: itemIdCol || columns[1],
          min_support: 0.05
        })
      });
  ...
  const handleAddToReport = () => {
    if (!result) return;
    addBlock('text', 'Market Basket Analysis', 'Generated Association Rules (Apriori).');
    if (result.rules) addBlock('data', 'Top Association Rules', result.rules.slice(0, 10));
      ...
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center h-[calc(100vh-100px)] text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 max-w-md mb-6">
            You need to upload a dataset to run Market Basket Analysis.
          </p>
  ...
  return (
    <DashboardLayout>
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Market Basket Analysis</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="col-span-1 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Transaction ID/Basket ID</label>
              <select value={transactionIdCol} onChange={(e) => setTransactionIdCol(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-700 dark:text-slate-300">
                <option value="">Select column...</option>
                {columns.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Item/Product ID</label>
              <select value={itemIdCol} onChange={(e) => setItemIdCol(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-700 dark:text-slate-300">
                <option value="">Select column...</option>
                {columns.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            ...
            >
              {isLoading ? 'Processing...' : <><Play size={16} /> Find Association Rules</>}
            </button>
          </div>
        </div>
        ...
        <div className="col-span-1 lg:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm flex items-center justify-center min-h-[400px]">
          {isLoading ? (
            <div className="text-center">
              ...
              <p className="text-slate-500">Calculating Support, Confidence, and Lift...</p>
            </div>
          ) : result ? (
             <div className="flex flex-col h-full w-full">
               <div className="flex items-center gap-3 mb-6">
                 ...
                 <div>
                   ...
                   <p className="text-sm text-slate-500">Apriori Association Rules</p>
                 </div>
               </div>
               ...
               {result.rules && result.rules.length > 0 ? (
                 <div className="overflow-x-auto border border-slate-200 dark:border-slate-700 rounded-lg">
                   <table className="w-full text-sm text-left">
                     ...
                     <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                       {result.rules.map((row: any, i: number) => (
                         <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                           <td className="px-4 py-3 font-semibold text-blue-600 dark:text-blue-400">{row.antecedents}</td>
                           <td className="px-4 py-3 font-semibold text-emerald-600 dark:text-emerald-400">{row.consequents}</td>
                           <td className="px-4 py-3">{row.support?.toFixed(4)}</td>
                           <td className="px-4 py-3">{(row.confidence * 100)?.toFixed(1)}%</td>
                           <td className="px-4 py-3 font-bold">{row.lift?.toFixed(2)}</td>
                         </tr>
                       ))}
                     </tbody>
                   </table>
                 ...
                 <div className="text-slate-500 text-center py-10">No association rules found. Try lowering the minimum support.</div>
               )}
             </div>
          ) : (
